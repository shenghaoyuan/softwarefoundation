...
Theorem minus_n_n : forall n,(*自减自为0*)
  minus n n = 0.
Proof.
  (* WORKED IN CLASS *)
  intros n. induction n as [| n' IHn'].
...
Proof.
  intros n m. induction n as [| n' IHn].
  -simpl.
   rewrite -> add_0_r. (*Lemma add_0_r : forall n : nat, n + 0 = n.*)
   reflexivity.
  -simpl. rewrite ->IHn.
   rewrite -> plus_n_Sm.
   reflexivity.
...
Proof.
  ...
  - simpl.
    reflexivity.
  - simpl.
    assumption. (*assumption 是一种策略，用于在证明中引入先前证明过的前提。*)
Qed.
(** [] *)
...
Proof.
  intros n m.
  assert (H: n + 0 + 0 = n).
    { rewrite add_comm. (*Theorem add_comm : forall n m : nat,
  n + m = m + n.*)
      simpl.
      rewrite add_comm. 
      reflexivity. }
...
(*Proof.
  ...
  rewrite ! IHb.
  rewrite ! plus_1_l.
  rewrite plus_n_Sm with(m := bin_to_nat b + 0).
  rewrite plus_Sn_m with(m := S (bin_to_nat b + 0)).
  reflexivity.
Qed.*)
